...
async function main() {
    console.log('🌱 [PRIMAL Seed] Starting warehouse database seeding...');
    ...
    console.log('🏗️ Seeding 20+ Racks with 3D Coordinates...');
    const rackDefinitions = [
        // Zone A Racks (X: -12 to -4, Z: -8 to 8)
        ...
        // Zone B Racks (X: -2 to 6, Z: -8 to 8)
        ...
        // Zone C Racks (X: 8 to 16, Z: -8 to 8)
    ...
    console.log('📦 Seeding Storage Locations (Bins)...');
    ...
    // 6. 50+ Products with Indian Pricing (₹) and Tech Specs
    console.log('📱 Seeding 50+ Products with Indian Pricing (₹)...');
    const rawProducts = [
        // Audio & Peripherals
        ...
        { sku: 'SSD-4001', name: 'HyperDrive 1TB NVMe PCIe 4.0 Internal SSD (7000MB/s)', cat: 'Computer Hardware', brand: 'HyperSilicon', price: 7299, cost: 4900, rating: 4.9, supplier: 'SiliconMatrix India', rop: 18, lead: 4, img: 'https://images.unsplash.com/photo-1597872200969-2b65d56bd16b?w=500' },
        ...
        // Additional Consumer Electronics to reach 50+
        ...
        { sku: 'AU-5010', name: 'AirDuo Wireless Lavalier Microphone Kit for Content Creators', cat: 'Audio & Peripherals', brand: 'VisionPro', price: 3699, cost: 1900, rating: 4.8, supplier: 'Visionix Electronics', rop: 22, lead: 4, img: 'https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=500' },
    ...
    // 7. 100+ Inventory Records across Racks & Bins
    console.log('📦 Seeding 100+ Inventory records across Racks & Bins...');
    const locationKeys = Object.keys(createdLocations);
    let locIdx = 0;
    for (let i = 0; i < createdProducts.length; i++) {
        const prod = createdProducts[i];
        // Primary warehouse stock (WH-A)
        const loc1 = createdLocations[locationKeys[locIdx % locationKeys.length]];
        locIdx++;
        // Specific deliberate scenarios:
        // Product 0 (Headphones) -> WH-A has 7 units, WH-B has 6 units (Perfect for Split Allocation Demo!)
        ...
        // Secondary Warehouse (WH-B) or Overflow stock for top items
        if (i < 20) {
    ...
    // 8. 5 Packing Stations with realistic queues
    console.log('📦 Seeding 5 Packing Stations...');
    await prisma.packingStation.createMany({
        data: [
            ...
            { stationCode: 'P03', workerName: 'Suresh Kumar', currentOrderNum: 'ORD-1048', queueDepth: 17, avgPackingTimeMin: 8.4, status: 'OVERLOADED' }, // Bottleneck Station!
    ...
    // 9. 40+ Orders in various stages with Indian customers
    console.log('🛒 Seeding 40+ Orders across fulfillment stages...');
    const customerNames = [
    ...
    for (let i = 0; i < 42; i++) {
        ...
        // SLA setup: high-priority orders have tight SLA
        ...
        // Add Picking task for orders in PICKING or PACKING
        ...
        // Add Shipment for orders in READY or DISPATCHED
    ...
    // 10. 20+ Exceptions with Resolution Intelligence
    console.log('⚠️ Seeding 20+ Exceptions...');
    ...
    console.log('✅ [PRIMAL Seed] Seeding completed successfully!');
}
